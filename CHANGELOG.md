...
- Enh: Custom side menu via `orcsis\admin\Module::items` (mdmunir).
...
- Chg: Remove `orcsis\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `orcsis\admin\Module::allowActions`. Set access control directly with `orcsis\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `orcsis\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
